...
public:
		double get_entropy_residue( std::vector<double> probability_residue);
		std::vector< std::vector < double > > probability_distribution( core::pose::Pose *population_ptr , const int population_size_ ,const int NUMBLOCk_ , std::string phi_psi);
		double get_sum_entropy( core::pose::Pose *population_ptr , const int population_size_);
		int find_individual_max_p( core::pose::Pose *population_ptr , const int population_size_);
		int find_worst_pose( core::pose::Pose *population_ptr , const int population_size_ , core::scoring::ScoreFunctionOP score_function );

private:
		std::vector< std::vector < int > > population_block_index ;
		std::vector< std::vector < double > > probability_dis_phi , probability_dis_psi;

//@miss77
public:
	char* format(double value);
	bool crossover( std::vector< core::pose::Pose > & population_, const Size index, core::pose::Pose &trial_);
	bool exchange_dihedral_angle( core::pose::Pose &target, core::pose::Pose &rand_conform, core::pose::Pose &trial_result, 
																const Size region_start, const Size length);
	bool Boltzmann_pose( core::pose::Pose &pose_before , core::pose::Pose &pose_late , core::scoring::ScoreFunctionOP score_function);
	bool Boltzmann_pose( core::pose::Pose &pose_before , core::pose::Pose &pose_late , contact_predicted& contact_);
	bool Boltzmann_pose( core::pose::Pose &pose_before , core::pose::Pose &pose_late , contact_predicted& contact_, core::scoring::ScoreFunctionOP score_function);
	bool boltzmann( double score_before , double score_late);
	bool get_average_rmsd_energy( std::vector< core::pose::Pose > & population_, Real &rmsd, Real &energy );
	//spicker
	void spicker_file(core::pose::Pose &pose );
	void output_spicker( core::pose::Pose &pose );
	bool spicker();
	bool output_status_info( struct tm * timeinfo_ , const int g_ , const int G_);
	bool output_status_info( char *start_time,  struct tm * timeinfo_2 , const int g_ , const int G_);
	void record_max_distance( const std::vector< core::pose::Pose > population_ , protocols::abinitio::contact_predicted &RaptorXcontact_);
	double get_max_distance_P( std::vector< core::pose::Pose > population_ , const std::vector<int> current_pair_ );
	void contact_dis( const std::vector< core::pose::Pose > population_ , protocols::abinitio::contact_predicted &RaptorXcontact_ );
	core::Real get_distance_paired( core::pose::Pose &pose , Size pos1, Size pos2 );
	void initiaize_contact_bin( int start_, int end_, int interval_);
	void reset_contact_bin( );
	bool count_num( std::vector< core::pose::Pose > population_, vector<int> current_pair_ );
	int prob_chose_bin( double contact_prob );
	vector<vector<int> > set_frag_range(std::vector<int> pertb_pair_ , int fragAss_size_ , core::pose::Pose pose);
	double entropy_contact_dis(Size population_size);
		
private:
	int file_num,count_decoy_spicker;
	vector<int> contact_bin_count;//the index of the vector is the distance bin - 4
};
